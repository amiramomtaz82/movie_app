...
class _MovieDetailsScreenState extends State<MovieDetailsScreen> {
  ...
  void initState() {
    ...
    context.read<FavoriteCubit>().loadFavoriteMovies(UserDM.currentUser!.id);
  }
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider.value(value: movieDetailsCubit),
        ...
        (builder :(context, state)
    {
      if (state.movieDetails.isLoading) {
        ...
        return SingleChildScrollView(
            ...
            ? Image.network(movie.largeCoverImage!, height: double.infinity,
              width: double.infinity,
              fit: BoxFit.cover,)
      ...
      BlocBuilder<FavoriteCubit, FavoriteState>(
        builder: (context, state) {
          ...
          if (state is FavoriteLoaded) {
            isFav = state.favoriteIds.contains(movie.id.toString());
          }
    ...
    context,
    MaterialPageRoute(
    builder: (context) => PlayMovie(url: movie.url!,movie: movie,),
    ),
    );
    }
